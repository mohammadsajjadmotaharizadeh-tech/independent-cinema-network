...
export default async function handler(req,res){
 ...
 const promptMode = (req.body && req.body.mode) || "default";
 const instructions = prompts[s.film] && prompts[s.film][promptMode] ? prompts[s.film][promptMode] : prompts[s.film];
 try{
  const r=await client.responses.create({model:process.env.OPENAI_MODEL,instructions:instructions,input:[...safe,{role:"user",content:String(message).slice(0,10000)}]});
